...
public class CalendarPickerView extends ListView {
    ...
    public CalendarPickerView(Context context, AttributeSet attrs) {
        ...
        setDivider(null);
        setDividerHeight(0);
        final int bg = getResources().getColor(ParcelUtil.getColorId("library_calendar_bg"));
        ...
        monthNameFormat = new SimpleDateFormat(context.getString(ParcelUtil.getStringId("library_month_name_format")), locale);
        weekdayNameFormat = new SimpleDateFormat(context.getString(ParcelUtil.getStringId("library_day_name_format")), locale);
    ...
    public FluentInitializer init(Date minDate, Date maxDate, Locale locale) {
        ...
        maxCal = Calendar.getInstance(locale);
        monthCounter = Calendar.getInstance(locale);
        monthNameFormat = new SimpleDateFormat(getContext().getString(ParcelUtil.getStringId("library_month_name_format")), locale);
        for (MonthDescriptor month : months) {
            month.setLabel(monthNameFormat.format(month.getDate()));
        }
        weekdayNameFormat = new SimpleDateFormat(getContext().getString(ParcelUtil.getStringId("library_day_name_format")), locale);
        fullDateFormat = DateFormat.getDateInstance(DateFormat.MEDIUM, locale);
        ...
        // maxDate is exclusive: bump back to the previous day so if maxDate is the first of a library_month,
        // we don't accidentally include that library_month in the view.
        maxCal.add(MINUTE, -1);
        ...
        while ((monthCounter.get(MONTH) <= maxMonth // Up to, including the library_month.
            ...
            Logr.d("Adding library_month %s", month);
            months.add(month);
...
//        smoothScrollToPosition(selectedIndex);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (months.isEmpty()) {
            throw new IllegalStateException(
                    "Must have at least one library_month to display.  Did you forget to call init()?");
        }
    ...
    /**
     * Hold a cell with a library_month-index.
     */
    ...
    /**
     * Return cell and library_month-index (for scrolling) for a given Date.
     */
    ...
    List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
        ...
        while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
            ...
            Logr.d("Building library_week row starting at %s", cal.getTime());
...
//            String errMessage = getResources().getString(R.string.library_invalid_date, fullDateFormat.format(minCal.getTime()), fullDateFormat.format(maxCal.getTime()));
            String errMessage = getResources().getString(ParcelUtil.getStringId("library_invalid_date"), fullDateFormat.format(minCal.getTime()), fullDateFormat.format(maxCal.getTime()));
