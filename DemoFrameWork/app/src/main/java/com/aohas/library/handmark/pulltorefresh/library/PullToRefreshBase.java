 ...
 *******************************************************************************/
package com.aohas.library.handmark.pulltorefresh.library;
...
import com.aohas.library.handmark.pulltorefresh.library.internal.FlipLoadingLayout;
import com.aohas.library.handmark.pulltorefresh.library.internal.LoadingLayout;
import com.aohas.library.handmark.pulltorefresh.library.internal.RotateLoadingLayout;
import com.aohas.library.handmark.pulltorefresh.library.internal.Utils;
import com.aohas.library.handmark.pulltorefresh.library.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// Styleables from XML
		TypedArray a = context.obtainStyledAttributes(attrs, ParcelUtil.getStyleableArray("PullToRefresh"));
		if (a.hasValue(ParcelUtil.getStyleableId("PullToRefresh", "ptrMode"))) {
			mMode = Mode.mapIntToValue(a.getInteger(ParcelUtil.getStyleableId("PullToRefresh", "ptrMode"), 0));
		}
//
		if (a.hasValue(ParcelUtil.getStyleableId("PullToRefresh", "ptrAnimationStyle"))) {
			mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(ParcelUtil.getStyleableId("PullToRefresh", "ptrAnimationStyle"), 0));
		}
		 ...
		 * Styleables from XML
		 */
		if (a.hasValue(ParcelUtil.getStyleableId("PullToRefresh", "ptrRefreshableViewBackground"))) {
			Drawable background = a.getDrawable(ParcelUtil.getStyleableId("PullToRefresh", "ptrRefreshableViewBackground"));
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		} else if (a.hasValue(ParcelUtil.getStyleableId("PullToRefresh", "ptrAdapterViewBackground"))) {
			Utils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
			Drawable background = a.getDrawable(ParcelUtil.getStyleableId("PullToRefresh", "ptrAdapterViewBackground"));
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		}

		if (a.hasValue(ParcelUtil.getStyleableId("PullToRefresh", "ptrOverScroll"))) {
			mOverScrollEnabled = a.getBoolean(ParcelUtil.getStyleableId("PullToRefresh", "ptrOverScroll"), true);
		}

		if (a.hasValue(ParcelUtil.getStyleableId("PullToRefresh", "ptrScrollingWhileRefreshingEnabled"))) {
			mScrollingWhileRefreshingEnabled = a.getBoolean(ParcelUtil.getStyleableId("PullToRefresh", "ptrScrollingWhileRefreshingEnabled"), false);
		}
	...
	public static enum Mode {
		...
		// The modeInt values need to match those from library_attrs.xml
		Mode(int modeInt) {
			mIntValue = modeInt;
		}
