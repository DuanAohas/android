...
public class SatelliteMenu extends FrameLayout {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        LayoutInflater.from(context).inflate(ParcelUtil.getLayoutId("library_sat_main"), this, true);
        imgMain = (ImageView) findViewById(ParcelUtil.getItemId("library_sat_main"));

        if (attrs != null) {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, ParcelUtil.getStyleableArray("SatelliteMenu"), defStyle, 0);
            satelliteDistance = typedArray.getDimensionPixelSize(ParcelUtil.getStyleableId("SatelliteMenu", "satelliteDistance"), DEFAULT_SATELLITE_DISTANCE);
            totalSpacingDegree = typedArray.getFloat(ParcelUtil.getStyleableId("SatelliteMenu", "totalSpacingDegree"), DEFAULT_TOTAL_SPACING_DEGREES);
            closeItemsOnClick = typedArray.getBoolean(ParcelUtil.getStyleableId("SatelliteMenu", "closeOnClick"), DEFAULT_CLOSE_ON_CLICK);
            expandDuration = typedArray.getInt(ParcelUtil.getStyleableId("SatelliteMenu", "expandDuration"), DEFAULT_EXPAND_DURATION);
    ...
    private void onClick() {
        if (plusAnimationActive.compareAndSet(false, true)) {
            ...
            if (itemClickedListener != null)
                itemClickedListener.itemsIsOpen(rotated);
    ...
    public void addItems(List<SatelliteMenuItem> items) {
        ...
        for (SatelliteMenuItem menuItem : menuItems) {
            ...
            ImageView itemView = (ImageView) LayoutInflater.from(getContext()).inflate(ParcelUtil.getLayoutId("library_sat_item_cr"), this, false);
            ImageView cloneView = (ImageView) LayoutInflater.from(getContext()).inflate(ParcelUtil.getLayoutId("library_sat_item_cr"), this, false);

            if (menuItem.getViewWidth() != 0 && menuItem.getViewHeight() != 0) {
                LayoutParams layoutParamsItem = getLayoutParams(itemView);
                LayoutParams layoutParamsClone = getLayoutParams(cloneView);
                layoutParamsItem.width = menuItem.getViewWidth();
                layoutParamsItem.height = menuItem.getViewHeight();
                layoutParamsClone.width = menuItem.getViewWidth();
                layoutParamsClone.height = menuItem.getViewHeight();
            }
    ...
    public void setMainSize(int width, int height) {
        ViewGroup.LayoutParams params = this.imgMain.getLayoutParams();
        params.width = width;
    ...
    public interface SateliteClickedListener {
        ...
        public void itemsIsOpen(boolean isOpen);
    }
